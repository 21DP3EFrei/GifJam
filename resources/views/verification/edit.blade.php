...
<div class="container mx-3">
    ...
    @endif
        <form id="myform" action="{{ route('verification.update', ['media' => $media->Me_ID]) }}" method="POST" class="mt-2 px-8 rounded-xl">
            @csrf
            @method('PUT')
            <div class="form-group mb-2">
                <label for="Nosaukums">{{ __('translation.media') }} {{ __('translation.name') }}:</label>
                <input title="{{ __('translation.titlefilename') }}" type="text" class="input input-md border rounded-sm bg-gray-200 dark:!bg-blue-900 dark:text-white dark:active:!bg-blue-900 dark:focus:!bg-blue-900 dark:focus:text-white autofill:!bg-black w-full" required oninvalid="this.setCustomValidity('{{ __('translation.fillFileName') }}')"oninput="this.setCustomValidity('')" autocomplete="off" id="Nosaukums" name="Nosaukums" value="{{ $media->Nosaukums }}" required>
            </div>
            <div class="form-group mb-2">
