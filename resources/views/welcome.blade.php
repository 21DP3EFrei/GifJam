...
@section('content')
<div class="container ml-8 flex flex-col w-auto">
  <h1 class="h1 break-words overflow-wrap pb-3" >{{ __('translation.welcome') }}</h1>
  @if (session('error'))
    <div class="alert alert-error mr-10 my-2">
        {{ session('error') }}
    </div>
  @endif
...
@if ($check)
<h1 class="text-2xl break-words overflow-wrap pt-5" >{{__('translation.previous')}}</h1>
    ...
    <table class="table overflow-x-auto rounded-box border border-base-content/5 bg-base-100 border-collapse text-sm">
      ...
      <tbody>
        <tr class="text-center"> <!-- Center-align all cells in this row -->
          <td class="align-middle items-center p-2">{{ $media->Nosaukums }}</td>
          <td class="p-2">
