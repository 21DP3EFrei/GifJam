            ...
            @foreach ($picture as $pictures)
                <div class="rounded-2xl bg-gray-200 dark:bg-gray-400 shadow-lg overflow-hidden">
                    <label class="swap swap-rotate">
                        <input 
                            type="checkbox" 
                            id="like-checkbox-{{ $pictures->Me_ID }}" 
                            onchange="toggleLike({{ $pictures->Me_ID }}, this.checked)" 
                            {{ Auth::user()->likeMedia($pictures) ? 'checked' : '' }} 
                        />
                        <!-- On -->
                        <svg 
                            id="like-icon-{{ $pictures->Me_ID }}" 
                            class="swap-on fill-current w-8 h-8 text-black ml-1 {{ Auth::user()->likeMedia($pictures) ? '' : 'hidden' }}" 
                        ...
                        <!-- Off -->
                        <svg 
                            id="unlike-icon-{{ $pictures->Me_ID }}" 
                            class="swap-off fill-current w-8 h-8 text-black bg-transparent ml-1 {{ Auth::user()->likeMedia($pictures) ? 'hidden' : '' }}" 
                            viewBox="0 0 24 24" 
                            fill="none" 
                            xmlns="http://www.w3.org/2000/svg"
                        >
                    ...
                    <!-- Image -->
                    <a href="{{ route('pictures.show', $pictures) }}">
