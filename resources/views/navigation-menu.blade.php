<nav x-data="{ open: false }" class="dark:border border-gray-300 dark:border-gray-600 outline-black dark:bg-gray-800"  aria-label="Main Navigation">
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex justify-between items-center py-4">
        ...
        <div class="hidden sm:flex space-x-7">
            ...
            <div x-data="{ open: false }" class="static text-left align-middle py-1 font-medium text-sm">
                <!-- Dropdown Trigger -->
                @if (request()->routeIs('categories.index') || request()->routeIs('sound-categories.index') || request()->routeIs('genre.index'))  
                <button @click="open = !open" type="button" class="pb-1 inline-flex items-center border-b-2 border-blue-400 text-sm font-medium leading-5 text-gray-900 dark:text-white focus:outline-hidden focus:border-indigo-700 focus:text-indigo-700 transition duration-150 ease-in-out no-underline">
                    @else
                    <button @click="open = !open" type="button" class="inline-flex items-center justify-center text-gray-500 dark:text-white">
                        @endif
                    Categories
                    <svg class="mr-1 h-5 w-5 dark:fill-white fill-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"  aria-hidden="true">
                        <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                    </svg>
                </button>
            
                <!-- Dropdown Menu -->
                <div x-show="open" @click.away="open = false" class="text-center flex items-center absolute mt-2 pl-2 w-56 bg-gray-100 dark:bg-gray-800! border rounded-xl z-50">
