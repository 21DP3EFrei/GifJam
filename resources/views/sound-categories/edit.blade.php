...
<div class="container mx-3">
        ...
        @endif
        <form id="myform" action="{{ route('sound-categories.update', ['SoundCategory' => $SoundCategory->SKat_ID]) }}" method="POST" class="mt-2 px-8 rounded-xl">
            @csrf
            @method('PUT')
            <div class="my-5 flex flex-col">
                <label for="Nosaukums">{{ __('translation.soundCategory') }} {{ __('translation.Name') }}:</label>
                <input title="{{ __('translation.titlecategoryname') }}" type="text" class="input input-md border rounded-sm bg-gray-200 dark:!bg-blue-900 dark:text-white dark:active:!bg-blue-900 dark:focus:!bg-blue-900 dark:focus:text-white autofill:!bg-black w-full" autocomplete="off" id="name" name="Kat_Nosaukums" value="{{ $SoundCategory->Nosaukums }}" required oninvalid="this.setCustomValidity('{{ __('translation.fillCategory') }}')" oninput="this.setCustomValidity('')">
            </div>
            <div class="my-5 flex flex-col">
                <label for="Apraksts">{{ __('translation.description') }}:</label>
                <textarea title="{{ __('translation.titlecategorydesc') }}" class="input input-md border rounded-sm bg-gray-200 dark:!bg-blue-900 dark:text-white dark:active:!bg-blue-900 dark:focus:!bg-blue-900 dark:focus:text-white autofill:!bg-black w-full" autocomplete="off" id="description" name="Apraksts">{{ $SoundCategory->Apraksts }}</textarea>
            </div>
            @if (Skana_kategorija::count() > 1)
            <div class="my-5 flex flex-col">
                <label for="apakskategorija">{{ __('translation.subcategoryOf') }} </label>
                <select title="{{ __('translation.subcategory') }}" class="select input-md border rounded-sm bg-gray-200 dark:!bg-blue-900 dark:text-white dark:active:!bg-blue-900 dark:focus:!bg-blue-900 dark:focus:text-white autofill:!bg-black w-full" id="apakskategorija" name="Apakskategorija">
                    <option value=""></option>
